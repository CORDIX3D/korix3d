...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const { data: product, error } = await supabase.from('products').select('name, short_description, description, images, meta_title, meta_description').eq('slug', slug).eq('active', true).maybeSingle();
...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: serializeJsonLd(productJsonLd) }} /><div className="mx-auto min-h-screen max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
