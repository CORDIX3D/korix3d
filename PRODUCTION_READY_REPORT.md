...
- Po publikacji bieżącej poprawki GitHub będzie zawierał 60 uporządkowanych migracji; lokalnie nadal pozostaje osobna migracja zabezpieczeń zastosowana już w produkcji, lecz niewłączona do tego commitu. Kontrola RLS obejmuje 44 tabele. Produkcja ma 51 tabel publicznych, wszystkie z RLS, 124 polityki i 4 wymagane buckety Storage.
...
| 3 | Supabase | 60 migracji po bieżącej publikacji i 1 oczekująca lokalnie, RLS, indeksy, FK, Storage i testy pgTAP | 51 tabel, 124 polityki, 0 tabel bez RLS, 4 buckety; poprawka pomijania anulowanych zadań działa w produkcji | Częściowo |
...
| 9 | Worker Creality | timeout, retry, heartbeat, panel produkcji, zadanie Windows, profile i zgodność 3MF | worker online; 3 rzeczywiste wyceny 3MF ukończone, w tym 2 po awarii CLI; odbiór STL, STEP i OBJ oczekuje | Częściowo |
...
| `npm test` | PASS | 5 plików, 54/54 testy |
...
| Automatyczna wycena Creality | PASS dla 3MF | worker online; dwa pliki `Cute Articulated Octopus.3mf`: 1,93 h, 37,5 g, 71,29 zł; kolejka 0/0/0 |
...
1. Opublikować w GitHub lokalną migrację `20260807130133_restrict_security_definer_execution.sql`, która została już zastosowana w produkcji.
2. Wykonać zewnętrzny, zaszyfrowany eksport bazy i Storage oraz próbne odtworzenie poza produkcją.
3. Naprawić historię wcześniejszych migracji oficjalnym `supabase migration repair`, aby przyszłe `db push` nie próbowało ich ponawiać.
4. Uzupełnić odbiór rzeczywistych modeli STL, STEP i OBJ; 3MF, retry, heartbeat oraz automatyczne uruchamianie workera są potwierdzone.
...
Gałąź `main` w GitHub przed bieżącą publikacją wskazuje commit `57b224a`. Ostatnie produkcyjne wdrożenie Vercel ma status `READY` i obsługuje aliasy `korix3d.pl`, `www.korix3d.pl` oraz `korix3d.vercel.app`. Bieżący zestaw zawiera naprawę automatycznej wyceny, migrację pomijającą anulowane zadania oraz aktualizację dokumentacji. Osobna migracja `20260807130133_restrict_security_definer_execution.sql` pozostaje poza tym commitem.
