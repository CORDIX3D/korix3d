...
export default async function MaterialDetailPage({ params }: { params: { slug: string } }) {
  ...
  return <div className="mx-auto min-h-screen max-w-5xl px-4 py-12">
    ...
    <div className="grid gap-10 lg:grid-cols-[1fr_1.4fr]">
      <div className="aspect-square overflow-hidden rounded-2xl bg-primary/10">{material.image_url ? <img src={material.image_url} alt={material.name} className="h-full w-full object-cover" /> : <div className="flex h-full items-center justify-center"><Layers className="h-24 w-24 text-primary/40" /></div>}</div>
