...
- GitHub zawiera 59 uporządkowanych migracji; lokalny workspace ma 60. migrację zabezpieczeń, zastosowaną już w produkcji, ale nadal oczekującą na publikację. Kontrola RLS obejmuje 44 tabele. Produkcja ma 51 tabel publicznych, wszystkie z RLS, 124 polityki i 4 wymagane buckety Storage.
...
- GitHub `main` jest zsynchronizowany z lokalnym kodem aplikacji, a automatyczne wdrożenie Vercel z `6624bee` ma status `READY` i obsługuje `korix3d.pl`.
...
| 3 | Supabase | 59 migracji opublikowanych i 1 oczekująca lokalnie, RLS, indeksy, FK, Storage i testy pgTAP | 51 tabel, 124 polityki, 0 tabel bez RLS, 4 buckety; uprawnienia funkcji produkcyjnych ograniczone i zweryfikowane | Częściowo |
...
| 9 | Worker Creality | timeout, retry, heartbeat, panel produkcji, aplikacja Windows/iPhone i profile | Creality Print 7.2.1 zainstalowany; WSL i Virtual Machine Platform włączone, ale restart i realny slicing czterech formatów oczekują | Blokada |
...
| `npm test` | PASS | 5 plików, 53/53 testy |
...
| pełne CI GitHub | PASS | run `31310692057` dla `6624bee` zakończony sukcesem |
...
4. Uruchomić ponownie Windows, dokończyć WSL i uruchomić worker z rzeczywistym Creality Print; odebrać STL, STEP, OBJ oraz 3MF.
...
Gałąź `main` w GitHub wskazuje commit `6624bee`. Produkcyjne wdrożenie Vercel `dpl_mz6KZWRoRwyZD7a7s7jrSwVG2ewW` ma status `READY` i obsługuje aliasy `korix3d.pl`, `www.korix3d.pl` oraz `korix3d.vercel.app`. Workflow GitHub `31310692057` zakończył się sukcesem. Lokalnie oczekuje tylko publikacja migracji zabezpieczeń Supabase oraz niniejszej aktualizacji raportu.
