-- Preserve the product cost at checkout time so later catalogue edits do not
-- rewrite historical margins.
...
SET unit_cost = round(GREATEST(COALESCE(product.cost_price, 0), 0)::numeric, 2)
FROM public.products AS product
WHERE item.product_id = product.id
  AND (item.unit_cost IS NULL OR item.unit_cost = 0);

UPDATE public.store_order_items
SET unit_cost = 0
WHERE unit_cost IS NULL OR unit_cost < 0 OR unit_cost = 'NaN'::numeric;
...
ALTER TABLE public.store_order_items
  DROP CONSTRAINT IF EXISTS store_order_items_unit_cost_check;
ALTER TABLE public.store_order_items
  ADD CONSTRAINT store_order_items_unit_cost_check
  CHECK (unit_cost >= 0 AND unit_cost <> 'NaN'::numeric) NOT VALID;

REVOKE SELECT ON public.store_order_items FROM authenticated;
GRANT SELECT (
  id, order_id, product_id, sku, name, quantity, unit_price, total, created_at
) ON public.store_order_items TO authenticated;
