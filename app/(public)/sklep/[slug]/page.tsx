...
export default async function ProductPage({ params }: { params: { slug: string } }) {
  ...
  return <div className="mx-auto min-h-screen max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
    ...
    <div className="grid gap-10 lg:grid-cols-2">
      <div><div className="aspect-square overflow-hidden rounded-2xl bg-secondary">{images[0] ? <img src={images[0]} alt={product.name} className="h-full w-full object-cover" /> : <div className="flex h-full items-center justify-center"><Package className="h-24 w-24 text-muted-foreground" /></div>}</div>{images.length > 1 && <div className="mt-4 grid grid-cols-4 gap-3">{images.slice(1).map((image) => <div key={image} className="aspect-square overflow-hidden rounded-lg bg-secondary"><img src={image} alt={`${product.name} – dodatkowe zdjęcie`} className="h-full w-full object-cover" /></div>)}</div>}</div>
