    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: `Jesteś doświadczonym dyrektorem zarządzającym (CEO) i dyrektorem finansowym (CFO) firmy produkcyjnej KORIX3D specjalizującej się w druku 3D.
Twoim zadaniem jest przygotowanie szczegółowego raportu wykonawczego w języku polskim.
Raport powinien być profesjonalny, konkretny i zawierać praktyczne rekomendacje.
WAŻNE: Wszystkie liczby i kwoty w raporcie musisz zaokrąglić do liczb całkowitych (bez miejsc po przecinku).`
          },
          {
            role: 'user',
            content: `Przygotuj raport wykonawczy dla miesiąca ${format(data.period.start, 'MMMM yyyy', { locale: pl })}.

DANE FINANSOWE:
- Przychód całkowity: ${Math.round(data.revenue.total)} PLN (zmiana: ${data.revenue.change.toFixed(1)}%)
- Przychód zamówienia 3D: ${Math.round(data.revenue.byType.orders3D)} PLN
- Przychód sklep: ${Math.round(data.revenue.byType.storeOrders)} PLN
- Koszty całkowite: ${Math.round(data.expenses.total)} PLN (zmiana: ${data.expenses.change.toFixed(1)}%)
- Zysk brutto: ${Math.round(data.profit.gross)} PLN
- Marża: ${data.profit.margin.toFixed(1)}%

DANE PRODUKCYJNE:
- Godziny pracy: ${Math.round(data.production.totalHours)} h
- Wykorzystanie maszyn: ${data.production.utilization.toFixed(0)}%
- Kolejka: ${data.production.queueSize} zamówień
- Liczba zamówień: ${data.orders.total}

DANE MAGAZYNOWE:
- Wartość magazynu: ${Math.round(data.warehouse.totalValue)} PLN
- Pozycji niskiego stanu: ${data.warehouse.lowStock}
- Filamenty niskiego stanu: ${data.filaments.lowStock.length}

DANE KLIENTÓW:
- Całkowita liczba: ${data.customers.total}
- Nowi klienci: ${data.customers.new}
- Retencja: ${data.customers.retentionRate.toFixed(0)}%

WYNIKI OCENY (0-100):
- Zdrowie finansowe: ${scores.financialHealth}
- Efektywność produkcji: ${scores.productionEfficiency}
- Zarządzanie magazynem: ${scores.warehouseManagement}
- Satysfakcja klientów: ${scores.customerSatisfaction}
- Wzrost biznesowy: ${scores.businessGrowth}
- OCENA OGÓLNA: ${scores.overallScore}

Przygotuj raport zawierający:
1. Podsumowanie wykonawcze (2-3 zdania o najważniejszych trendach)
2. Analiza przychodów (co było główną przyczyną zmian)
3. Analiza kosztów i rentowności
4. Ocena efektywności produkcyjnej
5. Stan magazynu i materiałów
6. Analiza bazy klientów
7. Konkretne rekomendacje na następny miesiąc`
          }
        ],
        max_tokens: 2500,
        temperature: 0.7
    ...
    if (response.ok) {
      ...
      return result.choices?.[0]?.message?.content || generateFallbackAnalysis(data, scores);
    }
  } catch (error) {
