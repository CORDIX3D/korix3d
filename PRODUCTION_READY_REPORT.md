...
- Lokalny zestaw zawiera 65 uporządkowanych migracji. Najnowsza migracja płatności wycen jest zastosowana i sprawdzona w produkcyjnym Supabase; 59 wcześniejszych wpisów nadal wymaga oficjalnego `migration repair`. Kontrola RLS obejmuje 44 tabele. Produkcja ma 51 tabel publicznych, wszystkie z RLS, 124 polityki i 4 wymagane buckety Storage.
- Zidentyfikowano 65 plików stron i 44 route handlery API.
...
| 4 | Stripe | checkout sklepu i wyceny, podpisany/idempotentny webhook, zwroty i dokumentacja | pełna płatność sklepu i refund PASS w Sandbox; płatność wyceny oraz zwrot materiału PASS w testach transakcyjnych Supabase; końcowy test po wdrożeniu oczekuje | Częściowo |
...
| Stripe Checkout wyceny 3D | PASS KOD/DB | osobne okno płatności, adres rozliczeniowy i wysyłkowy, anulowanie, potwierdzenie, refund oraz rezerwacja filamentu; dwa testy transakcyjne zakończone `ROLLBACK`; test przeglądarkowy po wdrożeniu oczekuje |
