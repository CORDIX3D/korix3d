  ...
  try {
    ...
    if (!parsed.success) {
      return NextResponse.json(
        { error: 'Nieprawidłowy identyfikator zamówienia.' },
    ...
    if (order.status !== 'paid' || !order.stripe_payment_intent_id) {
      return NextResponse.json(
        { error: 'Można zwrócić tylko w pełni opłacone zamówienie.' },
    ...
    const refund = await getStripeServer().refunds.create(
      ...
      { idempotencyKey: `korix3d-store-refund-${order.id}` }
    );
    ...
    console.error('Admin store order refund error:', error);
    return NextResponse.json(
